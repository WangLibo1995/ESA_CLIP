...
class DynamicWeightFusion(nn.Module):
    def __init__(self, in_dim):
        ...
        self.gap = nn.AdaptiveAvgPool2d((1, 1))
        self.channel_norm = nn.LayerNorm(in_dim * 3)
        self.conv1d = nn.Conv1d(1, 1, 3, padding=1)
        self.fc1 = nn.Linear(in_dim * 3, in_dim)
        self.fc2 = nn.Linear(in_dim, in_dim * 3)
        self.fc = nn.Sequential(
    ...
    def forward(self, x_rgb, x_swir):
        B, N, D = x_rgb.shape
        H = W = int((N - 1) ** 0.5)
        rgb_cls, swir_cls = x_rgb[:, 0, :], x_swir[:, 0, :]  # [B, D]
        rgb_tokens, swir_tokens = x_rgb[:, 1:, :], x_swir[:, 1:, :]  # [B, HW, D]

        rgb_feat = rgb_tokens.reshape(B, D, H, W)
        swir_feat = swir_tokens.reshape(B, D, H, W)
        combined = torch.cat([rgb_feat, swir_feat], dim=1)
        gate = self.gate(combined)
        img_fused = torch.addcmul(swir_feat, gate, rgb_feat - swir_feat)
        img_gap = self.gap(img_fused).view(B, D)
        img_feat = torch.cat([rgb_cls, img_gap, swir_cls], dim=1)   # [B, 3D]

        img_feat = self.channel_norm(img_feat)
        residual = img_feat
        global_feat= self.fc2(self.fc1(img_feat))
        local_feat = self.conv1d(img_feat.view(B, 1, 3*D)).view(B, 3*D)
...
class Adapter(nn.Module):
    ...
    def forward(self, x_rgb, x_swir):
        ...
        rgb_spectrum = self.spectrum_adapter1(q_swir, k_rgb, v_rgb)
        swir_spectrum = self.spectrum_adapter2(q_rgb, k_swir, v_swir)

        x = self.fusionizer(rgb_spectrum, swir_spectrum)
        return x
...
class ESA_CLIP(nn.Module):
    def __init__(
        ...
        self.clip, _, _ = create_model_and_transforms(model_name, pretrained=pretrained_name, force_image_size=256)
    ...
    def forward(self, img_rgb, img_swir):
        ...
        if self.use_adapter:
            ...
            image_features = self.adapter(rgb_tokens, swir_tokens)

        else:
            img_rgb_feat = self.clip.encode_image(img_rgb)  # (batch_size, 512)
            img_swir_feat = self.clip.encode_image(img_swir)
            image_features = img_rgb_feat + img_swir_feat
            image_features = self.fc_adapter(image_features)
        ...
        res = {"image_features": image_features, "text_features": text_features, "logits": logits}
        return res
